(() => {
  ...
  const boot = () => {
    ...
    async function openAlbum(album, user) {
      ...
      let onDetailKey = null;
      const closeDetail = () => { if (onDetailKey) document.removeEventListener("keydown", onDetailKey); box.remove(); document.body.style.overflow = ""; };
      box.querySelector(".album-modal-close").onclick = closeDetail; box.addEventListener("click", e => { if (e.target === box) closeDetail(); });
      onDetailKey = e => { if (e.key === "Escape" && !box.querySelector(".album-picker")) closeDetail(); };
      ...
      box.querySelector(".album-add-photos").onclick = () => {
        ...
        let onPickerKey = null;
        const closePicker = () => { if (onPickerKey) document.removeEventListener("keydown", onPickerKey); picker.remove(); };
        picker.querySelector(".album-modal-close").onclick = closePicker;
        picker.addEventListener("click", e => { if (e.target === picker) closePicker(); });
        onPickerKey = e => { if (e.key === "Escape") closePicker(); };
