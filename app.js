document.addEventListener("DOMContentLoaded", () => {
  ...
  const SUPABASE_KEY = "sb_publishable_djOS3_rIKhZ42gAXR5svKA_VAhqTrmt";
  const BUCKET = "photos";
  ...
  async function uploadOne(file, user) {
    ...
    // Idempotency guard: the same image can only have one database record for this user.
    ...
    if (uploadError) {
      // Another upload can win a race between the check above and storage upload.
    ...
    if (dbError) {
      ...
      // A concurrent request may have inserted the same record. Treat it as success, not as a duplicate.
  ...
  async function uploadPhotos(files) {
    // Set the lock BEFORE any await. This prevents two file-picker events from starting two upload batches.
    if (uploading) return;
    uploading = true;
    try {
      const { data: { user } } = await supabase.auth.getUser();
  ...
  loginButtons.forEach((b) => b.addEventListener("click", async (e) => {
    ...
    if (session) await supabase.auth.signOut(); else openAuth("login");
  }));
  ...
  authForm?.addEventListener("submit", async (e) => {
    ...
    try {
      if (authMode === "login") {
        ...
        if (error) throw error;
        if (data.session) closeAuth(); else setAuthMessage("Konto utworzone. Sprawdź e-mail, aby potwierdzić adres.", true);
      }
    } catch (err) { setAuthMessage(err.message || "Wystąpił błąd."); }
  ...
  supabase.auth.onAuthStateChange((event, session) => {
    updateAuthUI(session?.user ?? null);
    if (session?.user) setTimeout(loadSavedPhotos, 0); else { clearSavedPhotos(); setCount(0); }
  });
});