document.addEventListener("DOMContentLoaded", () => {
  ...
  // Verified legacy browser key. Keep this key for compatibility with the current Supabase JS client.
  const SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImpjbXdqbWF5d2ttnjrciziixIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODY4ODI4MjQsImV4cCI6MjEwMjQ1ODgyNH0.RtBREQD2r-shvaAJ9tSPR5ypYdIe9pqhwh4QJ3n-ruA";
  const BUCKET = "photos";

  const supabase = window.supabase?.createClient
    ? window.supabase.createClient(SUPABASE_URL, SUPABASE_KEY, {
        auth: { persistSession: true, autoRefreshToken: true, detectSessionInUrl: true, storageKey: "lap-chwile-auth-v2" }
      })
    : null;
  ...
  const updateAuthUI = (user) => loginButtons.forEach((button) => {
  ...
  if (!supabase) {
    ...
    console.error("Supabase JS SDK is not available.");
    return;
  }
  ...
  addButtons.forEach((button) => button.addEventListener("click", async (e) => {
    ...
    if (error) { setAuthMessage("Sesja wygasła. Zaloguj się ponownie."); return openAuth("login"); }
  ...
  authForm?.addEventListener("submit", async (e) => {
    ...
    if (!email || !password) return setAuthMessage("Podaj e-mail i hasło.");
    authSubmit.disabled = true;
    setAuthMessage("Logowanie…", true);
    try {
      if (authMode === "login") {
        const { data, error } = await supabase.auth.signInWithPassword({ email, password });
        if (error) throw error;
        if (!data?.session) throw new Error("Logowanie nie utworzyło sesji. Spróbuj ponownie.");
        closeAuth();
      ...
      setAuthMessage(String(err?.message || "Wystąpił błąd logowania."));
    } finally {
      authSubmit.disabled = false;
      authSubmit.textContent = authMode === "login" ? "Zaloguj się" : "Utwórz konto";
    }
  });
  ...
  (async () => {
    const { data: { session }, error } = await supabase.auth.getSession();
    if (error) { console.error("Session error:", error); updateAuthUI(null); return; }
  ...
  supabase.auth.onAuthStateChange((event, session) => {
    ...
    else { clearSavedPhotos(); setCount(0); }
  });
});
