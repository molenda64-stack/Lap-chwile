document.addEventListener("DOMContentLoaded", () => {
  ...
  async function deletePhoto(photoId, imagePath, card) {
    ...
    if (dbError) { alert("Nie udało się usunąć zdjęcia: " + dbError.message); return; }
    ...
    card?.remove();
    const remaining = gallery?.querySelectorAll('[data-saved-photo="true"]').length || 0;
    setCount(remaining);
  }
  ...
  function addImageToGallery(url, id, imagePath) {
    ...
    const button = document.createElement("button"); button.className = "delete-photo"; button.type = "button"; button.title = "Usuń zdjęcie"; button.setAttribute("aria-label", "Usuń zdjęcie"); button.textContent = "🗑";
  ...
  async function loadSavedPhotos() {
    ...
    clearSavedPhotos();
    data.forEach((photo) => { const { data: publicData } = supabase.storage.from(BUCKET).getPublicUrl(photo.image_path); addImageToGallery(publicData.publicUrl, photo.id, photo.image_path); });
    setCount(data.length);
  }

  async function uploadPhoto(file) {
    if (!file) return;
    if (!file.type.startsWith("image/")) return alert("Wybierz zdjęcie.");
    if (file.size > 10 * 1024 * 1024) return alert("Zdjęcie jest za duże. Maksymalny rozmiar to 10 MB.");
    const { data: { user }, error: userError } = await supabase.auth.getUser();
    if (userError || !user) return alert("Najpierw się zaloguj.");
    ...
    if (uploadError) return alert("Nie udało się dodać zdjęcia: " + uploadError.message);
    const { error: dbError } = await supabase.from("photos").insert({ user_id: user.id, image_path: filePath, caption: "" });
    if (dbError) { await supabase.storage.from(BUCKET).remove([filePath]); return alert("Nie udało się zapisać zdjęcia w bazie."); }
