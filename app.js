document.addEventListener("DOMContentLoaded", () => {
  ...
  function openAuth(mode = "login") {
    authMode = mode;
    authTitle.textContent = mode === "login" ? "Witaj w Łap Chwilę" : "Utwórz konto";
    authSubtitle.textContent = mode === "login" ? "Zaloguj się, aby zachowywać swoje wspomnienia." : "Załóż konto i zacznij zachowywać swoje chwile.";
    authSubmit.textContent = mode === "login" ? "Zaloguj się" : "Utwórz konto";
    authSwitch.textContent = mode === "login" ? "Nie masz konta? Zarejestruj się" : "Masz już konto? Zaloguj się";
    setAuthMessage(""); authForm?.reset(); authModal?.classList.add("open"); document.body.style.overflow = "hidden";
    setTimeout(() => authEmail?.focus(), 50);
  }
  function closeAuth() { authModal?.classList.remove("open"); document.body.style.overflow = ""; setAuthMessage(""); }

  if (!supabase) { updateAuthUI(null); setAuthMessage("Nie udało się uruchomić logowania. Odśwież stronę i spróbuj ponownie."); console.error("Supabase JS SDK is not available."); return; }
  ...
  viewer.addEventListener("click", (e) => { if (e.target === viewer || e.target.classList.contains("viewer-close")) closeViewer(); });
  const openViewer = (url) => { viewerImage.src = url; viewer.classList.add("open"); document.body.style.overflow = "hidden"; };

  async function deletePhoto(photoId, imagePath, card) {
    const { data: { user } } = await supabase.auth.getUser(); if (!user || !photoId || !imagePath) return;
    ...
    const { error: storageError } = await supabase.storage.from(BUCKET).remove([imagePath]); if (storageError) console.warn(storageError);
  ...
  async function loadSavedPhotos() {
    if (loadingPhotos) return; loadingPhotos = true;
    try {
      ...
      if (error) { console.error("photos select:", error); setUploadStatus("Nie udało się wczytać zapisanych chwil."); return; }
      clearSavedPhotos(); const seenPaths = new Set();
      data.forEach((photo) => { if (seenPaths.has(photo.image_path)) return; seenPaths.add(photo.image_path); const { data: publicData } = supabase.storage.from(BUCKET).getPublicUrl(photo.image_path); addImageToGallery(publicData.publicUrl, photo.id, photo.image_path); });
  ...
  async function uploadOne(file, user) {
    if (!file.type.startsWith("image/")) throw new Error("To nie jest zdjęcie."); if (file.size > 10 * 1024 * 1024) throw new Error(`${file.name}: maks. 10 MB.`);
    const hash = await sha256(file); const extension = (file.name.split(".").pop() || "jpg").toLowerCase().replace(/[^a-z0-9]/g, "") || "jpg"; const filePath = `${user.id}/${hash}.${extension}`;
    const { data: existing, error: existingError } = await supabase.from("photos").select("id").eq("user_id", user.id).eq("image_path", filePath).limit(1); if (existingError) throw existingError; if (existing?.length) return false;
  ...
  async function uploadPhotos(files) {
    ...
    try {
      const { data: { user } } = await supabase.auth.getUser(); if (!user) { openAuth("login"); return; }
      const selected = Array.from(files || []); const valid = selected.filter((f) => f.type.startsWith("image/") && f.size <= 10 * 1024 * 1024); if (!valid.length) { alert("Wybierz zdjęcia do 10 MB każde."); return; }
      setUploadStatus(`Przesyłanie 0/${valid.length} zdjęć…`); let done = 0, skipped = 0, failed = 0; const batchHashes = new Set();
      for (const file of valid) { try { const hash = await sha256(file); if (batchHashes.has(hash)) { skipped++; continue; } batchHashes.add(hash); const added = await uploadOne(file, user); if (added) done++; else skipped++; setUploadStatus(`Przesyłanie ${done + skipped}/${valid.length} zdjęć…`); } catch (error) { failed++; console.error(error); } }
      await loadSavedPhotos();
      if (failed) setUploadStatus(`Dodano ${done}. Pominięto ${skipped}. Błędy: ${failed}.`); else if (skipped) setUploadStatus(`Gotowe — dodano ${done}, pominięto ${skipped} duplikat${skipped === 1 ? "" : "y"}.`); else setUploadStatus(`Gotowe — dodano ${done} ${done === 1 ? "zdjęcie" : "zdjęć"}.`);
    } finally { uploading = false; }
  }

  addButtons.forEach((button) => button.addEventListener("click", async (e) => {
    e.preventDefault(); if (uploading) return; const { data: { user }, error } = await supabase.auth.getUser();
    if (error) { setAuthMessage("Sesja wygasła. Zaloguj się ponownie."); return openAuth("login"); } if (!user) return openAuth("login");
    const input = document.createElement("input"); input.type = "file"; input.accept = "image/*"; input.multiple = true; input.onchange = () => { if (input.files?.length) uploadPhotos(input.files); }; input.click();
  }));

  loginButtons.forEach((button) => button.addEventListener("click", async (e) => {
    e.preventDefault(); const { data: { session }, error } = await supabase.auth.getSession(); if (error) { openAuth("login"); return; } if (session) await supabase.auth.signOut(); else openAuth("login");
  }));
  ...
  authForm?.addEventListener("submit", async (e) => {
    e.preventDefault(); const email = authEmail.value.trim(), password = authPassword.value; if (!email || !password) return setAuthMessage("Podaj e-mail i hasło."); authSubmit.disabled = true; setAuthMessage("Logowanie…", true);
    try {
      if (authMode === "login") {
        const { data, error } = await supabase.auth.signInWithPassword({ email, password }); if (error) throw error; if (!data?.session) throw new Error("Logowanie nie utworzyło sesji. Spróbuj ponownie."); closeAuth(); updateAuthUI(data.user); await loadSavedPhotos();
      } else {
        const { data, error } = await supabase.auth.signUp({ email, password }); if (error) throw error;
        if (data.session) { closeAuth(); updateAuthUI(data.user); await loadSavedPhotos(); } else setAuthMessage("Konto utworzone. Sprawdź e-mail, aby potwierdzić adres.", true);
      }
    } catch (err) { console.error("Auth error:", err); setAuthMessage(String(err?.message || "Wystąpił błąd logowania.")); }
    finally { authSubmit.disabled = false; authSubmit.textContent = authMode === "login" ? "Zaloguj się" : "Utwórz konto"; }
  });

  document.addEventListener("keydown", (e) => { if (e.key === "Escape") { closeViewer(); closeAuth(); } });
  (async () => { const { data: { session }, error } = await supabase.auth.getSession(); if (error) { console.error("Session error:", error); updateAuthUI(null); return; } updateAuthUI(session?.user ?? null); if (session?.user) await loadSavedPhotos(); })();
